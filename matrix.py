...
def copy_matrix(matrix):
    return copy.deepcopy(
        matrix)  # we need to use deepcopy since we're dealing with 2D arrays
...
class Matrix:
    ...
    def translate(self, x, y, z):
        for i in range(4):
            self.data[3][i] = self.data[3][i] + (
                        self.data[0][i] * x + self.data[1][i] * y +
                        self.data[2][i] * z)
    ...
    def perspective(self, fovy, aspect, near, far):
        ...
        self.frustum(-frustum_x * near, frustum_x * near, -frustum_y * near,
                     frustum_y * near, near, far)
